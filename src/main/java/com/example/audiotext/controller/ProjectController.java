...
public class ProjectController {
 ...
 @GetMapping("/projects/{id}") public String details(@PathVariable Long id, Model m){ m.addAttribute("project",repo.findById(id).orElseThrow(() -> new IllegalArgumentException("Проект не найден: "+id))); m.addAttribute("recognitionMode", smart.currentMode()); m.addAttribute("exportFormats", java.util.List.of("txt","docx","pdf","json")); return "project-details"; }
