...
public class JdbcProjectRepository implements ProjectRepository {
    ...
    public Project save(Project p) { /* unchanged style compact */
        ...
        jdbc.update(con -> {
            var ps = con.prepareStatement("insert into projects(title,original_file_name,original_file_path,converted_file_path,status,created_at,updated_at) values(?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
            ps.setString(1, p.getTitle()); ps.setString(2, p.getOriginalFileName()); ps.setString(3, p.getOriginalFilePath()); ps.setString(4, p.getConvertedFilePath()); ps.setString(5, p.getStatus().name()); ps.setString(6, now.toString()); ps.setString(7, now.toString()); return ps;
        }, kh);
    ...
    public void update(Project p) { p.setUpdatedAt(LocalDateTime.now()); jdbc.update("update projects set title=?,status=?,updated_at=?,raw_text=?,processed_text=?,ai_text=?,error_message=?,duration_seconds=?,converted_file_path=? where id=?", p.getTitle(), p.getStatus().name(), p.getUpdatedAt().toString(), p.getRawText(), p.getProcessedText(), p.getAiText(), p.getErrorMessage(), p.getDurationSeconds(), p.getConvertedFilePath(), p.getId()); }
    ...
    private TextAnalysisResult loadAnalysis(Long projectId) { try { var list = jdbc.query("select * from analysis_results where project_id=? order by id desc limit 1", (rs, n) -> { TextAnalysisResult r = new TextAnalysisResult(); r.wordCount = rs.getInt("word_count"); r.sentenceCount = rs.getInt("sentence_count"); r.paragraphCount = rs.getInt("paragraph_count"); r.uniqueWordCount = rs.getInt("unique_word_count"); r.averageSentenceLength = rs.getDouble("average_sentence_length"); r.wordsPerMinute = rs.getDouble("words_per_minute"); try { r.keywordFrequency = mapper.readValue(rs.getString("keywords_json"), mapper.getTypeFactory().constructMapType(LinkedHashMap.class, String.class, Integer.class)); r.fillerWordFrequency = mapper.readValue(rs.getString("filler_words_json"), mapper.getTypeFactory().constructMapType(LinkedHashMap.class, String.class, Integer.class)); } catch (Exception ex) { r.keywordFrequency = new LinkedHashMap<>(); r.fillerWordFrequency = new LinkedHashMap<>(); } r.algorithmicSummary = rs.getString("algorithmic_summary"); return r; }, projectId); return list.isEmpty() ? null : list.get(0); } catch (Exception e) { return null; } }
    private Project map(ResultSet rs, int n) throws SQLException { Project p = new Project(); p.setId(rs.getLong("id")); p.setTitle(rs.getString("title")); p.setOriginalFileName(rs.getString("original_file_name")); p.setOriginalFilePath(rs.getString("original_file_path")); p.setConvertedFilePath(rs.getString("converted_file_path")); p.setStatus(ProjectStatus.valueOf(rs.getString("status"))); p.setRawText(rs.getString("raw_text")); p.setProcessedText(rs.getString("processed_text")); p.setAiText(rs.getString("ai_text")); p.setErrorMessage(rs.getString("error_message")); p.setDurationSeconds(rs.getDouble("duration_seconds")); p.setCreatedAt(parseDateTime(rs.getString("created_at"))); p.setUpdatedAt(parseDateTime(rs.getString("updated_at"))); return p; }
    private LocalDateTime parseDateTime(String value) { try { return value == null ? null : LocalDateTime.parse(value); } catch (Exception ex) { return null; } }
