...
public class JdbcProjectRepository implements ProjectRepository {
    ...
    public void saveAudioFile(Long projectId,String fileName,String contentType,byte[] fileData){ jdbc.update("delete from audio_files where project_id=?",projectId); jdbc.update("insert into audio_files(project_id,kind,file_name,content_type,file_size,file_data,created_at) values(?,?,?,?,?,?,?)",projectId,"ORIGINAL",fileName,contentType,fileData.length,fileData,LocalDateTime.now()); } public void upsertExportFile(Long projectId,ExportFormat format,String fileName,String contentType,byte[] fileData){ jdbc.update("delete from export_files where project_id=? and format=?",projectId,format.name()); jdbc.update("insert into export_files(project_id,format,file_name,content_type,file_size,file_data,created_at) values(?,?,?,?,?,?,?)",projectId,format.name(),fileName,contentType,fileData.length,fileData,LocalDateTime.now()); } public Optional<ProjectRepository.ExportedFile> findExportFile(Long projectId,ExportFormat format){ var list=jdbc.query("select file_name,content_type,file_data from export_files where project_id=? and format=?",(rs,n)->new ProjectRepository.ExportedFile(rs.getString("file_name"),rs.getString("content_type"),rs.getBytes("file_data")),projectId,format.name()); return list.stream().findFirst(); } public void deleteById(Long id){ jdbc.update("delete from projects where id=?",id);}
