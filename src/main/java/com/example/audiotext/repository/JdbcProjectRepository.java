...
public class JdbcProjectRepository implements ProjectRepository {
 ...
 private TextAnalysisResult loadAnalysis(Long projectId){ try{ var list=jdbc.query("select * from analysis_results where project_id=? order by id desc limit 1",(rs,n)->{ TextAnalysisResult r=new TextAnalysisResult(); r.wordCount=rs.getInt("word_count"); r.sentenceCount=rs.getInt("sentence_count"); r.paragraphCount=rs.getInt("paragraph_count"); r.uniqueWordCount=rs.getInt("unique_word_count"); r.averageSentenceLength=rs.getDouble("average_sentence_length"); r.wordsPerMinute=rs.getDouble("words_per_minute"); r.keywordFrequency=mapper.readValue(rs.getString("keywords_json"),mapper.getTypeFactory().constructMapType(java.util.LinkedHashMap.class,String.class,Integer.class)); r.fillerWordFrequency=mapper.readValue(rs.getString("filler_words_json"),mapper.getTypeFactory().constructMapType(java.util.LinkedHashMap.class,String.class,Integer.class)); r.algorithmicSummary=rs.getString("algorithmic_summary"); return r;},projectId); return list.isEmpty()?null:list.get(0);}catch(Exception e){return null;} }
