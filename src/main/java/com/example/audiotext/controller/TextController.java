...
public class TextController {
    ...
    private void recalc(com.example.audiotext.model.Project p, String username){ var a=new TranscriptionResult(); a.setDurationSeconds(p.getDurationSeconds()!=null?p.getDurationSeconds():0); var selected=TextVersionSelector.selectedText(p); var result=analysis.analyze(selected.text(),a,username,p.getRawText()); result.sourceTextType=selected.type().name(); p.setAnalysisResult(result); repo.update(p); repo.updateAnalysis(p.getId(),p.getAnalysisResult()); }
}
